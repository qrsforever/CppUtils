/***************************************************************************
 *  SQLiteDatabase.cpp - SQLiteDatabase Impl
 *
 *  Created: 2018-06-21 10:32:08
 *
 *  Copyright QRS
 ****************************************************************************/
...
int SQLiteDatabase::load(const char *filepath)
{
    //Using file store
    int rc;
    sqlite3 *fileDB;
    ...
    if (SQLITE_OK == rc) {
        pBackup = sqlite3_backup_init(mSQLiteDB /* to */, "main", fileDB /* from */, "main");
        if (pBackup) {
...
int SQLiteDatabase::save(const char *filepath)
{
    //Using file store
    int rc;
    sqlite3 *fileDB;
    ...
    rc = sqlite3_open(filepath, &fileDB);
    if (SQLITE_OK == rc) {
        pBackup = sqlite3_backup_init(fileDB /* to */, "main", mSQLiteDB /* from */, "main");
        if (pBackup) {
            sqlite3_backup_step(pBackup, -1);
            sqlite3_backup_finish(pBackup);
        }
...
bool SQLiteDatabase::tableExists(std::string name)
{
    int count = 0;
